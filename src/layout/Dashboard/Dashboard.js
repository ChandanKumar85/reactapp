...
export default function Dashboard() {
  ...
  // console.log("🚀 ~ file: Dashboard.js:105 ~ Dashboard ~ user:", user.name)

  const userDetail = JSON.parse(localStorage.getItem('user'))

  const { token, logout } = Authentication()
            ...
            >
              {
                userDetail?.profileImg ?
                  <Avatar sx={{ width: 40, height: 40 }} alt={userDetail.name} src={userDetail?.profileImg} /> :
                  <Avatar sx={{ width: 40, height: 40 }}>{userDetail.name.charAt(0).toUpperCase()}</Avatar>
              }
            </IconButton>
            <Menu
              anchorEl={anchorEl}
